...
int main(int ac, char **av) {
	...
	if (fscanf(f, "%d %c %c %c\n", &height, &empty, &obs, &full) != 4) // check if the map example or subject file args are seperated by spaces or not, if yes leave at is, if not seperated remove the spaces between the call in scanf "%d%c%c%c" .....
